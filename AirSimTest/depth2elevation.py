...
def acquire_data(client):
    ...
    return depth_img, state.kinematics_estimated.position, state.kinematics_estimated.orientation
...
def process_depth_data(client, depth_img, position, orientation, max_depth):
    ...
    # 世界坐标系转换
    rot = Rotation.from_quat([orientation.x_val, orientation.y_val,
    ...
    points_world = (R_inv @ points_world.T).T - drone_pos
...
def main():
    ...
    try:
        ...
        np.savetxt("elevation_points.csv", points_world, delimiter=",")
